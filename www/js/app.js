   ...
   switch(page_id)
   {
	  ...
	  case "address_form_select": 
	   ...
	   if(location_mode==1){
        ...
        $active_merchant_id = getActiveMerchantIDCheckout();  
        params_city+='&merchant_id='+$active_merchant_id;
        ...
        $active_merchant_id = getActiveMerchantIDCheckout();
        params_area+='&merchant_id='+$active_merchant_id;
        ...
        processDynamicAjax('StateList', 'merchant_id='+$active_merchant_id ,'location_state_result_loader','GET',1 ); 
        ...
        $( "#field_location_state" ).keyup(function( event ) {
   	    	if ( event.which == 13 ) {
				...
				if(!empty(search_field_by_name)){
					data = "search_str="+ search_field_by_name;	
					data+='&merchant_id='+$active_merchant_id;
    	...
    	if(data.code==1){    		    		
	        ...
	        case "getOrderHistory2":	  	              
	          ...
	          case "loadCart":
	            ...
	            case "subscribeAlertToMerchant":	              
	              var subscribeBtn = document.getElementById("subscribeBtn");   
	              ons.modifier.toggle(subscribeBtn, 'to_text_orange');
	              $("#subscribeBtn").attr("onclick",'unsubscribeAlertToMerchant()');
	            break;
	            
	            case "unsubscribeAlertToMerchant":	              
	              var subscribeBtn = document.getElementById("subscribeBtn");   
	              ons.modifier.toggle(subscribeBtn, 'to_text_orange');
	              ons.modifier.add(subscribeBtn, 'text_to_black');
	              $("#subscribeBtn").attr("onclick",'subscribeAlertToMerchant()');
	            break;
...
payNowNextStep = function(data){
	...
	switch (data.details.next_step){
		...
		case "receipt":		
		  ...
		  $mtid = getActiveMerchantID();		  
		  AutoSubscribeAlertToMerchant($mtid);
		...
		case "init_jad":	
		   ...
		   onsenNavigator.pushPage('jad_form.html',{
			  	animation : "slide",
			  	data : {
			  		order_id : data.details.order_id,
			  		total_amount : data.details.total_amount,
			  		first_name : data.details.client_info.first_name,
			  		last_name : data.details.client_info.last_name
			  	}
		  });  
...
showLocationCity = function(){
	state_id = parseInt($('.state_id').val());		
	if(state_id>0){
...
getActiveMerchantIDCheckout = function(){
	$found_cart = false;
	$.each(onsenNavigator.pages, function($pagekey, $pageval){        	
        if($pageval.id=="cart"){
        	$found_cart=true;
        }
    });
...
subscribeAlertToMerchant = function($merchant_id){
	...
	ons.platform.select('ios');  
    ons.notification.confirm( t("Subscribe alert to this merchant, you will receive promo,news,events and other exciting offer") ,{
		title: t("Subsribe"),
		id : "subscribe_alert",
		buttonLabels : [ t("Yes"), t("Cancel") ]
	}).then(function(input) {
		if (input==0){			
			...
			if(!isdebug()){
				window.FirebasePlugin.subscribe("merchant_subscription_"+merchant_id, function(){
			        showToast("Succesfully subscribe to alert");
			        processDynamicAjax('subscribeAlertToMerchant',"merchant_id="+merchant_id,'order_loader','GET' ); 
			    },function(error){
			        showToast( t("Failed to subscribe to alert"), error);
			    });
			} else {
...
unsubscribeAlertToMerchant = function($merchant_id){
	try {
		...
		if(!isdebug()){
			window.FirebasePlugin.unsubscribe("merchant_subscription_"+merchant_id, function(){		        
				showToast("Succesfully subscribe to alert");
		        processDynamicAjax('unsubscribeAlertToMerchant',"merchant_id="+merchant_id,'order_loader','GET' ); 
		    },function(error){
		        showToast( t("Failed to unsubscribe to alert") , error);
		    });
		} else {
			showToast("Succesfully unsubscribe to alert");
			processDynamicAjax('unsubscribeAlertToMerchant',"merchant_id="+merchant_id,'silent','GET' ); 
		}
...
AutoSubscribeAlertToMerchant = function($merchant_id){
...
subscribeButton = function(is_added){
	html='';
	if(is_added){
	   html+='<ons-toolbar-button onclick="unsubscribeAlertToMerchant()" modifier="to_text_orange" >';
          html+='<ons-icon icon="md-notifications"  size="25px" ></ons-icon>';
       html+='</ons-toolbar-button>';
	} else {
		html+='<ons-toolbar-button onclick="subscribeAlertToMerchant()" >';
          html+='<ons-icon icon="md-notifications-none"  size="25px" ></ons-icon>';
       html+='</ons-toolbar-button>';
	}
	return html;
}
