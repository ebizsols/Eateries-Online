...
fillRestoPageInfo = function(data){
	...
	merchant_subscribe = false;
	if ((typeof  data.merchant_subscribe !== "undefined") && ( data.merchant_subscribe !== null)) {
	  	   merchant_subscribe = data.merchant_subscribe;
	}	       	
	...
	html+='<ons-row>';
	   ...
	   html+='<ons-col vertical-align="center" class="text-right">';
	     if(merchant_subscribe){
	     	html+='<ons-button id="subscribeBtn" onclick="unsubscribeAlertToMerchant()" modifier="quiet to_text_orange" >';
	     } else {
	     	html+='<ons-button id="subscribeBtn" modifier="quiet text_to_black" onclick="subscribeAlertToMerchant()" >';
	     }	    
              html+='<ons-icon icon="md-notifications"  size="22px" ></ons-icon>';
	       html+='</ons-button>';
	   html+='</ons-col>';	  
	html+='</ons-row>';
	...
	html+='<ons-col vertical-align="top" class="text-right raty-medium">';	
...
var displayCartDetails = function(datas){
	...
	if ((typeof data.total.service_fee !== "undefined") && ( data.total.service_fee !== null)) {
	if (!empty(data.total.service_fee)){
		if(data.total.service_fee>0.0001){
		   html+= twoColumn( t('Service Fee') ,  prettyPrice(data.total.service_fee) );
		}
	}
	}
...
setBookingList = function(data, element){
	...
	$.each( data  , function( key, val ) {
		...
		html+='<div class="inner">';
		   ...
		   if ((typeof  val.booking_remarks !== "undefined") && ( val.booking_remarks !== null)) {
		   if(!empty(val.booking_remarks)){
		      html+='<p class="resp_small" style="padding-top:0;">'+ val.booking_remarks+'</p>';
		   }
		   }
